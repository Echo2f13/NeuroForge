...
class TestImageLoaderValidation:
    ...
    def test_file_not_found(self, tmp_path):
        with pytest.raises(FileNotFoundError):
            self.loader.load(str(tmp_path / "nonexistent.png"))

    def test_unsupported_format(self, tmp_path):
        gif_file = tmp_path / "image.gif"
        gif_file.write_bytes(b"fake")
        with pytest.raises(UnsupportedImageFormatError):
            self.loader.load(str(gif_file))

    @pytest.mark.parametrize("ext", [".png", ".jpg", ".jpeg", ".bmp", ".tiff", ".tif"])
    def test_valid_extensions_accepted(self, tmp_path, ext):
        """Validation should pass for all supported extensions."""
        img_file = tmp_path / f"test{ext}"
        img_file.write_bytes(b"fake image data")
        result = ImageLoader._validate_file(str(img_file))
...
class TestSortBoxesReadingOrder:
    def test_empty_results(self):
        assert ImageLoader._sort_boxes_reading_order([]) == []

    def test_single_result(self):
        results = [([[0, 0], [100, 0], [100, 30], [0, 30]], "hello", 0.95)]
        sorted_r = ImageLoader._sort_boxes_reading_order(results)
...
class TestDiagramDetection:
    def test_no_results_suggests_diagram(self):
        ...
        assert ImageLoader._detect_diagrams([], 1000, 800) is True
    ...
    def test_dense_text_no_diagram(self):
        ...
        assert ImageLoader._detect_diagrams(results, 1000, 800) is False
    ...
    def test_sparse_text_suggests_diagram(self):
        ...
        assert ImageLoader._detect_diagrams(results, 2000, 2000) is True
    ...
    def test_scattered_boxes_suggest_diagram(self):
        ...
        assert ImageLoader._detect_diagrams(results, 1000, 1000) is True
...
class TestOCRFallback:
    ...
    @patch.object(ImageLoader, "_ocr_with_paddleocr", return_value=None)
    @patch.object(ImageLoader, "_ocr_with_tesseract", return_value=None)
    def test_no_engine_available_raises_error(self, mock_tess, mock_paddle, tmp_path):
        img = tmp_path / "test.png"
        img.write_bytes(b"fake png data")
        with patch.object(ImageLoader, "_validate_file", return_value=img):
            with pytest.raises(OCREngineError):
                self.loader.load(str(img))

    @patch.object(ImageLoader, "_get_image_dimensions", return_value=(800, 600))
    @patch.object(ImageLoader, "_ocr_with_tesseract")
    @patch.object(ImageLoader, "_ocr_with_paddleocr")
...
class TestImageLoaderDocument:
    def setup_method(self):
        self.loader = ImageLoader()

    @patch.object(ImageLoader, "_get_image_dimensions", return_value=(1024, 768))
    @patch.object(ImageLoader, "_ocr_with_paddleocr")
    ...
    @patch.object(ImageLoader, "_get_image_dimensions", return_value=(1000, 1000))
    @patch.object(ImageLoader, "_ocr_with_paddleocr")
    ...
    @patch.object(ImageLoader, "_get_image_dimensions", return_value=(500, 500))
    @patch.object(ImageLoader, "_ocr_with_paddleocr")
...
# ---------------------------------------------------------------------------
# Integration: Export from __init__.py
# ---------------------------------------------------------------------------


class TestExports:
    def test_image_loader_importable_from_package(self):
        """ImageLoader should be importable from src.ingestion."""
        from src.ingestion import ImageLoader as IL

        assert IL is ImageLoader
